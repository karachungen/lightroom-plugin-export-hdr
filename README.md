...
Lightroom Classic **export filter** (macOS) plus `**uhdr_repack`**: one Ultra HDR (gain-map) **JPEG** from your normal export and an internal HDR TIFF, using [google/libultrahdr](https://github.com/google/libultrahdr).
...
- **Lightroom Classic 13+** — `[Info.lua](ExportHDR.lrplugin/Info.lua)` (`LrSdkMinimumVersion = 13.0`). Export filter: **Ultra HDR (uhdr_repack)**.
