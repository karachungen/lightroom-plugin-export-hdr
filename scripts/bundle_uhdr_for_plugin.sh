#!/usr/bin/env bash
# Build uhdr_repack (macOS arm64) and install into ExportHDR.lrplugin/bin with bundled dylibs.
set -euo pipefail
...
if [[ "$(uname -m)" != "arm64" ]]; then
	echo "Warning: bundling for arm64; this Mac is not arm64. Pass -DCMAKE_OSX_ARCHITECTURES=arm64 may fail or cross-compile incorrectly." >&2
fi

echo "==> Configuring CMake (Release, arm64, macOS 13.0+)"
...
if ! file "$BUILD_EXE" | grep -q "arm64"; then
	echo "Warning: $BUILD_EXE does not look like arm64. Check your toolchain." >&2
fi
