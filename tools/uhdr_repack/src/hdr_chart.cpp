...
int write_hdr_chart_main(const std::string& dir) {
  ...
  if (!write_float_tiff(tiff, hdr, kWidth, kHeight, tiff_icc, &err)) {
    std::cerr << err << "\n";
    return 1;
  }
  std::vector<uint8_t> jpeg_bytes;
  if (!encode_p3_jpeg(sdr.data(), kWidth, kHeight, jpeg_icc, &jpeg_bytes, &err) ||
      !write_bytes(jpeg, jpeg_bytes, &err)) {
    std::cerr << err << "\n";
    return 1;
  }
  {
