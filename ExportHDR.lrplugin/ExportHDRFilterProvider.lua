				...
				f:row {
					...
					f:static_text {
						...
						title = table.concat({
							...
							"Use HDR editing in Develop when needed (Lightroom 13+).",
						}, "\n"),
						height_in_lines = 5,
					},
				},
...
function ExportHDRFilterProvider.postProcessRenderedPhotos(functionContext, filterContext)
	...
	if not CMD.binaryExists(binary) then
		local msg = "uhdr_repack not found at:\n"
			.. binary
			.. "\n\nRun scripts/bundle_uhdr_for_plugin.sh (macOS arm64) to install bin/uhdr_repack inside the plug-in bundle."
	...
	for sourceRendition, _renditionToSatisfy in filterContext:renditions() do
		...
		if not hdrPath or not LrFileUtils.exists(hdrPath) then
			...
			error(
				"Ultra HDR: HDR TIFF render failed. Requires Lightroom Classic 13+ with HDR editing/export support.\n"
